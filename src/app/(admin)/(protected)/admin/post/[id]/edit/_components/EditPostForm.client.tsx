...
import { updatePostAction, deletePostAction } from "@features/posts/actions";
...
const EditPostForm = ({ categories, post }: EditPostFormProps) => {
  ...
  const [selectedCategoryId, setSelectedCategoryId] = useState<string>(String(post.categoryId));
  ...
  const { mutateAsync: updatePostAsync, isPending: isSaving } = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["posts"] });
      toast.success(isPublished ? "발행 상태로 저장되었습니다." : "수정되었습니다.");
      router.push(`/admin/post/${post.id}`);
      router.refresh();
    },
    onError: (error) => toast.error(error instanceof Error ? error.message : "저장 중 오류가 발생했습니다."),
  });
  ...
  const handleDeletClick = () => {
    dialog.confirm({
      ...
      onConfirm: () => { void deletePostAsync(); },
    });
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-[720px] mx-auto">
      <div className="flex items-center justify-between">
        <Link href={`/admin/post/${post.id}`}><Button variant="outline"><Icon name="ArrowLeft" /> 뒤로</Button></Link>
        <div className="flex gap-2">
          <Button variant="destructive" disabled={isDeleting} onClick={handleDeletClick}>
            {isDeleting ? <Spinner className="size-4" /> : <Icon name="Trash2" />} 삭제
          </Button>
          <Button onClick={() => void handleSubmit()} disabled={isSaving}>
            {isSaving ? <Spinner className="size-4" /> : <Icon name="Save" />} 저장
          </Button>
        </div>
      </div>
